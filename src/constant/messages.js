...
const OUTPUT_MESSAGE = Object.freeze({
  purchaseAmout: (amount) => "\n${amount}개를 구매했습니다.",
  winStatistic: "\n당첨통계\n---",
  totalWinsStatistic: (label, conut) => "$(label) - $(conunt)개",
...
const STATISTIC_LABEL = Object.freeze({
  ...
  [PRIZE_KEY.secondPrize]: "${RANK.second}개 일치, 보너스 볼 일치 (30,000,000)",
  [PRIZE_KEY.firstPrize]: "${RANK.first}개 일치 (2,000,000,000)",
});
...
const ERROR_MESSAGE = Object.freeze({
  invalidLength:
    "${PREFIX_ERROR} 로또 번호는 ${LOTTO_CONDITION.length}개여야 합니다.",
  invalidNumberic: "$(PREFIX_ERROR} 숫자가 잘못된 형식입니다.",
  invalidRange:
    "${PREFIX_ERROR} 로또 번호는 ${LOTTO_CONDITION.startRange}부터 ${LOTTO_CONDITION.endRange}사이의 숫자여야 합니다.",
  invalidUnique: "${PREFIX_ERROR} 로또 번호는 중복되지 않아야 합니다.",
  invalidAmout:
    "$(PREFIX_ERROR} 구입 금액은 ${PRICE_PER_TICKET}원으로 나누어 떨어져야 합니다.",
  invalidBonusNumber: "${PREFIX_ERROR} 보너스 번호는 하나의 숫자만 가능합니다.",
  invalidUniqueBonusNumber:
    "${PREFIX_ERROR} 보너스 번호는 로또 번호와 중복되지 않아야 합니다.",
});
