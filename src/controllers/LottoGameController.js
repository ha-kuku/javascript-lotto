...
class LottoGameController {
  ...
  async startGame() {
    ...
    const winsStatistics = this.#calculateWinsStatistics(
      winningLotto,
      bonusNumber
    );
  ...
  async #requestValidBonusNumber(winningLotto) {
    while (true) {
      try {
        ...
        const isContaining = winningLotto.hasContainBonusNumber(
  ...
  #printResults(winsStatistics) {
    ...
    const totalPrizeAmount =
      this.#lottoGame.getTotalPrizeAmount(winsStatistics);
